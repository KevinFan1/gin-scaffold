...
func JWTMiddleWareGenerator() *jwt.GinJWTMiddleware {
	middleware, err := jwt.New(&jwt.GinJWTMiddleware{
		...
		Unauthorized:    UnAuthorizationHandler,
...
func IdentityHandler(c *gin.Context) interface{} {
	...
	userId := int(claims[JWTIdentityKey].(float64))
	...
	return &UserJwtClaims{
		...
		SessionId: claims[JWTSessionKey],
	}
...
func LoginHandler(c *gin.Context) (interface{}, error) {
	...
	//更新user对象 last_login
	go global.DB.Model(&user).Update("last_login", time.Now())
...
func UnAuthorizationHandler(c *gin.Context, code int, message string) {
	// 上述函数校验失败返回内容
	switch code {
